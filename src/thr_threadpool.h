...
#ifndef LMP_THR_THREADPOOL_H
#define LMP_THR_THREADPOOL_H

#include "error.h"
#include "fix_threadpool.h"    // IWYU pragma: export
#include "pointers.h"
...
class ThrThreadpool {
 ...
 protected:
  ...
  FixThreadpool *fix;    // pointer to fix_omp;

  const int thr_style;
  int thr_error;

 public:
  ThrThreadpool(LAMMPS *, int);
  virtual ~ThrThreadpool() = default;
